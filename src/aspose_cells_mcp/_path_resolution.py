...
Governs *local disk* access only: `cells_download_workbook`'s local output-file write is the one
Cells tool that touches the local filesystem. Cells' single-call upload+convert operation,
`Workbook_PutConvertWorkBook`, sends the file as the raw request body with no named parameter at all
(a shape Codegen doesn't support yet) and was deferred (Sprint 11 execution plan) — unlike Words'
`words_convert_to_format`, so Cells has no local *input*-reading tool. Every other Cells tool's
